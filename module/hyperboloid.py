...
class Hyperboloid():
    ...
    def proj(self, x, c):
        ...
        K = 1. / c
        d = x.size(-1) - 1
        y = x.narrow(-1, 1, d)      #🌟y: [41342, 49]
        y_sqnorm = torch.norm(y, p=2, dim=1, keepdim=True) ** 2 #🌟y: [41342, 1]
        mask = torch.ones_like(x)
        mask[:, 0] = 0              #🌟mask[i]: [0, 1, 1, ... 1], shape: [50]
        vals = torch.zeros_like(x)
        vals[:, 0:1] = torch.sqrt(torch.clamp(K + y_sqnorm, min=self.eps[x.dtype])) 
            #🌟torch.clamp: replace value lower than min with min, self.eps is define at 'init'
        return vals + mask * x
